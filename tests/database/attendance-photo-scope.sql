...
select lives_ok($$with v as(select jsonb_build_object('kind','session','actor',auth.uid(),'time',extract(epoch from now()),'session','97000000-0000-4000-8000-000000000001','scope',public.attendance_photo_scope('97000000-0000-4000-8000-000000000001'),'revision',0,'roster_token',public.attendance_register('97000000-0000-4000-8000-000000000001')->>'roster_token','sha256',repeat('b',64),'matched','[]'::jsonb,'exceptions','[]'::jsonb,'processing_ms',1) p) select public.attendance_photo_accept(p::text,pg_temp.proof(p)) from v$$,'First photo has no approved reference match');
select public.attendance_reference_approve('94000000-0000-4000-8000-000000000001','99000000-0000-4000-8000-000000000088');
select lives_ok($$with v as(select jsonb_build_object('kind','session','actor',auth.uid(),'time',extract(epoch from now()),'session','97000000-0000-4000-8000-000000000001','scope',public.attendance_photo_scope('97000000-0000-4000-8000-000000000001'),'revision',1,'roster_token',public.attendance_register('97000000-0000-4000-8000-000000000001')->>'roster_token','sha256',repeat('b',64),'matched','["99000000-0000-4000-8000-000000000001"]'::jsonb,'exceptions','[]'::jsonb,'processing_ms',1) p) select public.attendance_photo_accept(p::text,pg_temp.proof(p)) from v$$,'Same image is reprocessed after reference approval');
select is(public.attendance_register('97000000-0000-4000-8000-000000000001')->'marks'->>'99000000-0000-4000-8000-000000000001','present','Newly approved match is not discarded as duplicate');
select lives_ok($$with v as(select jsonb_build_object('kind','session','actor',auth.uid(),'time',extract(epoch from now()),'session','97000000-0000-4000-8000-000000000001','scope',public.attendance_photo_scope('97000000-0000-4000-8000-000000000001'),'revision',2,'roster_token',public.attendance_register('97000000-0000-4000-8000-000000000001')->>'roster_token','sha256',repeat('b',64),'matched','["99000000-0000-4000-8000-000000000001"]'::jsonb,'exceptions','[]'::jsonb,'processing_ms',1) p) select public.attendance_photo_accept(p::text,pg_temp.proof(p)) from v$$,'Same scope replay remains idempotent');
reset role;select is((select count(*)::int from private.attendance_photo_receipts where session_id='97000000-0000-4000-8000-000000000001'),2,'Both scoped processing receipts retained without duplicate replay');select * from finish();rollback;
