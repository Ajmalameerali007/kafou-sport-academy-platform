-- Reviewed additive corrections: preserve original migration and all history.
alter table private.delivery_starts add column revision bigint not null default 0;
create or replace function public.daily_command(p_action text,p_data jsonb,p_key uuid) returns jsonb language plpgsql security definer set search_path='' as $$
...
begin
 ...
 if p_action='shift.in' then
  ...
  if sh.clocked_out_at is null and (sh.clocked_in_at at time zone 'Asia/Dubai')::date<(now() at time zone 'Asia/Dubai')::date then raise exception 'This open shift needs a reviewed time correction' using errcode='P0409';end if;
  ...
  if found then rid:=ds.id;else
   ...
   if sh.id is not null and (sh.clocked_in_at at time zone 'Asia/Dubai')::date<(now() at time zone 'Asia/Dubai')::date then raise exception 'This open shift needs a reviewed time correction' using errcode='P0409';end if;
 ...
 elsif p_action='session.correct_start' then
  select * into s from public.class_sessions where id=(p_data->>'session_id')::uuid for update;
  select * into cl from public.academy_classes where id=s.class_id;
  perform private.require_access(private.operations_staff(cl.branch_id));
  select * into ds from private.delivery_starts where session_id=s.id for update;
  if ds.id is null or s.delivered_at is not null or s.status='cancelled' or ds.revision is distinct from (p_data->>'revision')::bigint or length(why)<5 then raise exception 'Review an undelivered session and record a reason' using errcode='P0409';end if;
  select coalesce((select coach_id from public.coach_substitutions where session_id=s.id and revoked_at is null order by created_at desc limit 1),cl.coach_id) into employee;
  select * into sh from private.staff_shifts where employee_id=employee and branch_id=cl.branch_id and clocked_out_at is null and (clocked_in_at at time zone 'Asia/Dubai')::date=(now() at time zone 'Asia/Dubai')::date;
  update private.delivery_starts set coach_id=employee,actor_id=auth.uid(),shift_id=sh.id,reason=why,revision=revision+1 where id=ds.id;rid:=ds.id;
  ...
  if ag.id is null or dt is null or dt<>date_trunc('month',dt)::date or ag.effective_from>=dt+interval '1 month' or ag.effective_to<=dt then raise exception 'Choose a month overlapping this agreement' using errcode='22023';end if;
  amt:=ag.amount_minor;v_allocations:=ag.allocations;
  if ag.effective_from>dt or ag.effective_to<dt+interval '1 month' then
   amt:=(p_data->>'reviewed_amount_minor')::bigint;v_allocations:=p_data->'allocations';
   if amt is null or amt<=0 or length(why)<5 then raise exception 'Enter a reviewed partial-month amount and reason' using errcode='22023';end if;
  ...
  if rid is null then
   insert into private.cost_obligations(kind,employee_id,agreement_id,period_start,cost_date,title,category,amount_minor,adjustment_minor,adjustment_reason,allocations,created_by)
    values('salary',ag.employee_id,ag.id,dt,dt,'Monthly salary','Salary',ag.amount_minor,amt-ag.amount_minor,nullif(why,''),v_allocations,auth.uid()) returning id into rid;
  end if;
...
create or replace function public.daily_read(p_view text,p_branch uuid default null,p_from date default (now() at time zone 'Asia/Dubai')::date,p_to date default (now() at time zone 'Asia/Dubai')::date,p_offset integer default 0,p_query text default '',p_id uuid default null) returns jsonb
...
begin
 ...
 'open_shift',(select to_jsonb(x)||jsonb_build_object('needs_review',(clocked_in_at at time zone 'Asia/Dubai')::date<(now() at time zone 'Asia/Dubai')::date) from private.staff_shifts x where employee_id=auth.uid() and clocked_out_at is null));
 if p_view='session' then
  perform private.require_access(private.attendance_access(p_id));
  return meta||jsonb_build_object('can_review_delivery',exists(select 1 from public.class_sessions s join public.academy_classes c on c.id=s.class_id where s.id=p_id and private.operations_staff(c.branch_id)),
 'expected_coach_id',(select coalesce((select coach_id from public.coach_substitutions where session_id=p_id and revoked_at is null order by created_at desc limit 1),c.coach_id) from public.class_sessions s join public.academy_classes c on c.id=s.class_id where s.id=p_id),
 'delivery',(select to_jsonb(d) from private.delivery_starts d where session_id=p_id),'delivered_at',(select delivered_at from public.class_sessions where id=p_id),'arrivals',(select coalesce(jsonb_agg(to_jsonb(a)),'[]') from private.student_arrivals a join public.session_roster r on r.id=a.roster_id where r.session_id=p_id));
  ...
  with scoped as (select s.*,(s.clocked_out_at is null and (s.clocked_in_at at time zone 'Asia/Dubai')::date<(now() at time zone 'Asia/Dubai')::date) needs_review,p.name employee_name,b.name branch_name from private.staff_shifts s join public.profiles p on p.id=s.employee_id left join public.branches b on b.id=s.branch_id
