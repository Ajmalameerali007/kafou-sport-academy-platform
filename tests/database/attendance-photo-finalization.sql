...
select lives_ok($$select public.attendance_draft_command('97000000-0000-4000-8000-000000000001','save',2,public.attendance_register('97000000-0000-4000-8000-000000000001')->>'roster_token','{"99000000-0000-4000-8000-000000000001":"present","99000000-0000-4000-8000-000000000002":"late"}',gen_random_uuid())$$,'Resolve remaining trial without rewriting the automatic mark');
select lives_ok($$select public.attendance_draft_command('97000000-0000-4000-8000-000000000001','finish',3,public.attendance_register('97000000-0000-4000-8000-000000000001')->>'roster_token','{}','90000000-0000-4000-8000-000000000099')$$,'Finish automatic register');
select set_config('request.jwt.claims','{"sub":"91000000-0000-4000-8000-000000000002","role":"authenticated","aal":"aal1"}',true);
select public.attendance_photo_consent('94000000-0000-4000-8000-000000000001',false);
select set_config('request.jwt.claims','{"sub":"91000000-0000-4000-8000-000000000001","role":"authenticated","aal":"aal2"}',true);
select lives_ok($$select public.attendance_draft_command('97000000-0000-4000-8000-000000000001','finish',3,public.attendance_register('97000000-0000-4000-8000-000000000001')->>'roster_token','{}','90000000-0000-4000-8000-000000000099')$$,'Lost successful finish response remains retrievable after later consent withdrawal');
select lives_ok($$select public.attendance_request_review('97000000-0000-4000-8000-000000000001','99000000-0000-4000-8000-000000000002','Please check this arrival time','90000000-0000-4000-8000-000000000044')$$,'Correction request on finalized attendance');
select lives_ok($$select public.attendance_request_review('97000000-0000-4000-8000-000000000001','99000000-0000-4000-8000-000000000002','Please check this arrival time','90000000-0000-4000-8000-000000000044')$$,'Correction request retry is idempotent');
select is(jsonb_array_length(public.attendance_reviews('97000000-0000-4000-8000-000000000001')->'items'),1,'Only one correction request exists');
